...
def main ():
    ...
    def render_deepdream(t_obj, img0=img_noise, iter_n = 10, step=1.5, octave_n=4,octave_scale=1.4):
        ...
        #split the image into a number of octave
        img = img0
        octaves = []
        ...
        for octave in range (octave_n):
            ...
            for _ in range (iter_n):
                g = calc_grad_tiled(img_t_grad)
                img += g*(step / np.abs(g).mean()+1e-7)
            #Step 4 - output deep dreamed image
            showarray(img/255.0)
    ...
    #step 4 - Apply gradient to that layer
    render_deepdream(T(layer)[:,:,:,139], img0)
    
    #Higher level deep dream
